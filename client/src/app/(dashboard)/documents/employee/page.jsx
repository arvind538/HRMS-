...
import { Loader2, FileText, Trash2, FolderOpen, Edit3, UserCheck, Calendar, ExternalLink, ShieldCheck, Award, CreditCard, Briefcase, Plus } from "lucide-react";
...
export default function DocumentCategoryPage({
  ...
  description = "Personal identification, verification documents, educational certificates, and bank proofs.",
  ...
  // Backend Data Fetching Function (Smooth & Optimized)
    ...
    <div className="space-y-6 sm:space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-300">
            ...
            return (
              ...
              >
                <div>
                  ...
                  <h3 className="text-xs sm:text-sm font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">{tmpl.title}</h3>
              ...
              <div className="flex items-start sm:items-center gap-3.5 sm:gap-4 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-400 mt-1 flex items-center gap-2 flex-wrap">
                    {d.employee?.name && (
                      <span className="inline-flex items-center gap-1 font-semibold text-indigo-700 bg-indigo-50 px-2.5 py-0.5 rounded-md border border-indigo-100/60 shadow-2xs">
                ...
                {isAdmin && (
                  <>
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-xl transition-all duration-300 shadow-2xs active:scale-95 group/edit cursor-pointer"
                      title="Edit Document"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-rose-600 bg-rose-50 hover:bg-rose-600 hover:text-white rounded-xl transition-all duration-300 shadow-2xs active:scale-95 group/del border border-rose-100 cursor-pointer"
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingDocId ? "Edit Employee Document" : "Upload Employee Document"}>
        <form onSubmit={handleUpload} className="space-y-4 pt-2">
          ...
          <div>
            <label className="text-xs font-bold uppercase tracking-wider text-slate-500">File URL (Cloud Link - Cloudinary / AWS)</label>
            <input
              required
              type="url"
              value={form.fileUrl}
