...
export default function AddEmployeePage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(err.response?.data?.message || "Failed to add employee record. Please try again.");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6 antialiased">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/80 shadow-xs">
                <div className="flex items-center gap-3.5">
                    <button
                        type="button"
                        onClick={() => router.back()}
                        className="p-2.5 rounded-2xl bg-slate-50 hover:bg-slate-100 text-slate-600 border border-slate-200 shadow-xs transition-all active:scale-95 shrink-0"
                        title="Go back"
                    >
                        <ArrowLeft size={18} />
                    </button>
            ...
            <div className="bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-8 shadow-xs transition-all duration-200">
                {error && (
                    <div className="flex items-center gap-2.5 bg-rose-50 border border-rose-200/80 text-rose-700 text-xs sm:text-sm px-4 py-3 rounded-2xl mb-6 shadow-xs animate-shake">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex flex-col-reverse sm:flex-row items-center justify-end gap-3 pt-4 border-t border-slate-100">
                        <button
                            type="button"
                            onClick={() => router.back()}
                            className="w-full sm:w-auto px-5 py-3 rounded-2xl border border-slate-200 text-slate-700 hover:bg-slate-50 text-xs sm:text-sm font-bold transition-all active:scale-95"
                        >
                            Cancel
                        </button>
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 rounded-2xl bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white text-xs sm:text-sm font-bold shadow-sm shadow-indigo-600/20 disabled:opacity-60 transition-all active:scale-95"
                        >
                            {loading ? (
                                <>
