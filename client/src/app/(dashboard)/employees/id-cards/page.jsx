...
export default function EmployeeIDCardsPage() {
    ...
    // Reliable download method using browser print-to-image or fallback dialog
    const handleDownloadCardImage = async (cardId, fileName) => {
        ...
        try {
            ...
            // Thoda pause taaki DOM elements fully paint ho sakein
            ...
            console.warn("Canvas generation skipped, triggering print dialog instead:", err);
            window.print();
        } finally {
    ...
    if (loading) {
        return (
            <div className="w-full py-44 flex flex-col items-center justify-center gap-3 text-slate-500">
    ...
    return (
        <div className="max-w-[1400px] mx-auto p-4 sm:p-6 lg:p-8 space-y-8 antialiased">
            <style jsx global>{`
                @media print {
                    body * {
                        visibility: hidden;
            ...
            {/* Control Deck */}
            <div className="no-print bg-white p-6 sm:p-5 rounded-3xl border border-slate-200/90 shadow-sm flex flex-col md:flex-row items-center justify-between gap-5">
                <div className="space-y-1 w-full md:w-auto text-left">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-slate-400 font-medium">
                            CR80 Dual-Sided Layout
                        </span>
                    </div>
                    <h1 className="text-2xl sm:text-2xl font-bold text-slate-900 tracking-tight">
                        {COMPANY_NAME} {COMPANY_SUB}
                    </h1>
                    <p className="text-sm sm:text-sm text-slate-500 font-bold">
                        Generate official smart ID passes with live QR verification codes and custom photo upload.
                    </p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
                    <div className="relative w-full sm:w-80">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs sm:text-sm text-slate-900 focus:bg-white focus:outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10 transition-all shadow-2xs"
                        />
                        {search && (
                            <button
                                type="button"
            ...
            {/* Cards Grid */}
            <div className="no-print grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 justify-items-center">
                {filteredEmployees.map((emp, index) => {
                    ...
                    return (
                        <div
                            key={empId}
                            className="flex flex-col items-center gap-3 transition-transform hover:-translate-y-1 duration-200"
                        >
                            <div
                                onClick={() => {
                                    setSelectedEmp(emp);
                            ...
                            >
                                <div className="absolute top-0 left-0 right-0 h-40 overflow-hidden pointer-events-none z-0">
                                    <svg viewBox="0 0 310 160" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full object-cover">
                                        <path d="M120 0H310V110C270 140 200 135 155 110C100 80 40 110 0 85V0H120Z" fill="#0066FF" />
                                        <path d="M160 0H310V85C270 110 210 105 160 85V0Z" fill="#0044CC" opacity="0.7" />
                                    </svg>
                                </div>
                                ...
                                <div className="relative z-10 pt-4 px-5 flex items-center gap-2">
                                    ...
                                    <div className="text-left leading-none">
                                        <h2 className="text-[12px] font-black tracking-wider text-white uppercase">{COMPANY_NAME}</h2>
                                        <p className="text-[6.5px] font-bold text-sky-100 uppercase tracking-tight mt-0.5">{COMPANY_SUB}</p>
                                    </div>
                                </div>
                                    ...
                                    <div className="w-[125px] h-[125px] p-1.5 bg-white rounded-2xl shadow-md border-2 border-[#1E293B]">
                                        <div className="w-full h-full rounded-xl bg-slate-100 overflow-hidden flex items-center justify-center">
                                            {emp.avatar || emp.photo ? (
                                                <img src={emp.avatar || emp.photo} alt={fullName} className="w-full h-full object-cover" />
                                            ) : (
                                                <span className="text-3xl font-black text-blue-600">{fullName.charAt(0).toUpperCase()}</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="relative z-10 text-center px-4 mt-0.5">
                                    <h3 className="text-[15px] font-black text-[#0066FF] uppercase tracking-tight truncate">{fullName}</h3>
                                    <div className="inline-block mt-1 px-3 py-0.5 bg-[#1E293B] rounded-md shadow-xs">
                                        <span className="text-[9.5px] font-extrabold text-white uppercase tracking-wider">{designation}</span>
                                    </div>
                                </div>
                                ...
                                <div className="relative z-10 px-6 space-y-1.5 text-[11px] font-bold text-[#1E293B]">
                                    <div className="grid grid-cols-[55px_10px_1fr] items-center">
                                        <span className="text-slate-500 uppercase font-extrabold text-[10px]">ID No</span>
                                        <span>:</span>
                                        <span className="font-black text-slate-900 tracking-wide">{token}</span>
                                    </div>
                                    <div className="grid grid-cols-[55px_10px_1fr] items-center">
                                        <span className="text-slate-500 uppercase font-extrabold text-[10px]">Email</span>
                                        <span>:</span>
                                        <span className="truncate text-[10.5px] font-semibold text-slate-700">{emp.email || "staff@4paysave.com"}</span>
                                    </div>
                                    <div className="grid grid-cols-[55px_10px_1fr] items-center">
                                        <span className="text-slate-500 uppercase font-extrabold text-[10px]">Phone</span>
                                        <span>:</span>
                                        <span className="font-semibold text-slate-700">{emp.phone || "+91 00000 00000"}</span>
                                    </div>
                                </div>

                                <div className="relative w-full h-20 overflow-hidden mt-1 flex items-end justify-between px-5 pb-2">
                                    <div className="absolute inset-0 pointer-events-none z-0">
                                        <svg viewBox="0 0 310 80" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full object-cover">
                                            <path d="M0 40C80 60 200 10 310 20V80H0V40Z" fill="#0066FF" opacity="0.15" />
                                        </svg>
                                    </div>
                                    ...
                                    <div className="relative z-10 text-right pb-1">
                                        <p className="text-[9px] font-extrabold text-slate-400 italic font-serif">Together</p>
                                        <p className="text-[11px] font-bold text-slate-600 italic font-serif -mt-1">Forever</p>
                                    </div>
                                </div>
                            </div>

                            <div className="w-[310px] flex items-center justify-between gap-2 px-1">
                                <button
                                    type="button"
                                    onClick={(e) => handleOpenEdit(emp, e)}
                                    className="flex-1 py-1.5 px-3 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-colors cursor-pointer shadow-2xs"
                                >
                                    <Edit3 size={13} />
                                    <span>Edit</span>
                                </button>
                                <button
                                    ...
                                    className="flex-1 py-1.5 px-3 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 rounded-xl text-xs font-bold shadow-2xs transition-colors cursor-pointer"
                                >
                                    Preview
                                </button>
                                <button
                                    ...
                                    className="py-1.5 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold shadow-sm flex items-center gap-1.5 transition-colors cursor-pointer"
                                >
                                    <Printer size={13} />
                                    <span>Print</span>
                                </button>
            ...
            {filteredEmployees.length === 0 && (
                <div className="bg-white rounded-3xl border border-dashed border-slate-300 p-16 text-center text-slate-500 text-xs font-semibold">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col">
                        <div className="p-5 border-b border-slate-100 flex items-center justify-between">
                            <div>
                                <h3 className="text-base font-black text-slate-900">Edit ID Card Details</h3>
                                <p className="text-xs text-slate-500">Update photo, custom ID No, and position settings.</p>
                            </div>
                            <button
                                type="button"
                        ...
                        <form onSubmit={handleSaveEmployee} className="p-5 space-y-3.5 overflow-y-auto max-h-[75vh]">
                            <div className="flex flex-col items-center justify-center gap-1.5 pb-1">
                                <div className="relative w-24 h-24 rounded-xl bg-slate-100 border-2 border-dashed border-slate-300 p-1 flex items-center justify-center overflow-hidden group shadow-inner">
                                    {editFormData.avatar ? (
                                        <img src={editFormData.avatar} alt="Preview" className="w-full h-full object-cover rounded-lg" />
                                    ) : (
                                ...
                                <button type="button" onClick={() => fileInputRef.current?.click()} className="text-xs font-bold text-blue-600 hover:text-blue-700 underline">
                                    Select Square Photo from Device
                                </button>
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                                ...
                                <div>
                                    <label className="font-bold text-slate-700 mb-1 block">ID No (Editable e.g. FPS0001)</label>
                                    <input
                                        type="text"
                                        required
                                ...
                                <div>
                                    <label className="font-bold text-slate-700 mb-1 block">Job Position / Designation</label>
                                    <input
                                        type="text"
                                        required
                                ...
                                <div>
                                    <label className="font-bold text-slate-700 mb-1 block">Contact Phone</label>
                                    <input
                                        type="text"
                                ...
                                <div>
                                    <label className="font-bold text-slate-700 mb-1 block">Official Email</label>
                                    <input
                                        type="email"
                            ...
                            <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-2.5">
                                <button type="button" onClick={() => setEditingEmp(null)} className="px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-semibold cursor-pointer">
                                    Cancel
                                </button>
                                <button type="submit" disabled={saving} className="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-sm cursor-pointer disabled:opacity-50">
                                    {saving ? <Loader2 size={13} className="animate-spin" /> : <Save size={13} />}
                                    <span>{saving ? "Updating..." : "Save & Update"}</span>
                                </button>
                            </div>
                        </form>
                    </div>
            ...
            {selectedEmp && (() => {
                ...
                return (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
                        <div className="relative w-full max-w-xl bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col md:flex-row">
                            <button
                                type="button"
                                onClick={() => setSelectedEmp(null)}
                                className="no-print absolute top-3.5 right-3.5 p-1.5 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 transition-colors z-30"
                            >
                                <X size={15} />
                            </button>

                            {/* Print / Preview Target */}
                            <div className="p-5 bg-slate-50 border-b md:border-b-0 md:border-r border-slate-200 flex flex-col items-center justify-center">
                                {!isFlipped ? (
                                    /* FRONT CARD */
                                    <div
                                    ...
                                    >
                                            ...
                                            <div className="w-[125px] h-[125px] p-1.5 bg-white rounded-2xl shadow-md border-2 border-[#1E293B]">
                                                <div className="w-full h-full rounded-xl bg-slate-100 overflow-hidden flex items-center justify-center">
                                                    {selectedEmp.avatar || selectedEmp.photo ? (
                                                        <img src={selectedEmp.avatar || selectedEmp.photo} alt={fullName} className="w-full h-full object-cover" />
                                                    ) : (
                                                        <span className="text-3xl font-black text-blue-600">{fullName.charAt(0).toUpperCase()}</span>
                                                    )}
                                                </div>
                                        ...
                                        <div className="relative z-10 px-6 space-y-1.5 text-[11px] font-bold text-[#1E293B]">
                                            ...
                                            <div className="grid grid-cols-[55px_10px_1fr] items-center">
                                                <span className="text-slate-500 uppercase font-extrabold text-[10px]">Email</span>
                                                <span>:</span>
                                                <span className="truncate text-[10.5px] font-semibold text-slate-700">{selectedEmp.email || "staff@4paysave.com"}</span>
                                            </div>
                                            <div className="grid grid-cols-[55px_10px_1fr] items-center">
                                                <span className="text-slate-500 uppercase font-extrabold text-[10px]">Phone</span>
                                                <span>:</span>
                                                <span className="font-semibold text-slate-700">{selectedEmp.phone || "+91 00000 00000"}</span>
                                            </div>
                                        </div>
                                    ...
                                    >
                                        <div className="absolute top-0 right-0 w-40 h-32 pointer-events-none z-0 overflow-hidden">
                                            <svg viewBox="0 0 160 120" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full object-cover">
                                                <path d="M60 0H160V100C130 110 90 90 60 60V0Z" fill="#0066FF" opacity="0.8" />
                                            </svg>
                                        </div>
                                        ...
                                        <div className="relative z-10 space-y-4 pt-2">
                                            ...
                                            <ul className="text-[11px] font-semibold space-y-2.5 text-slate-700 list-disc pl-4">
                                                <li>This ID card is the property of {COMPANY_NAME} {COMPANY_SUB}.</li>
                                                <li>This card is strictly non-transferable.</li>
                                                <li>Carry this badge at all times during working hours.</li>
                                                <li>If found, please return to company HR immediately.</li>
                                            </ul>
                                        </div>
                                        ...
                                        <div className="relative z-10 space-y-3 pb-2 text-center">
                                            <div className="flex flex-col items-center">
                                                ...
                                                <p className="text-[11px] font-extrabold text-slate-900 tracking-tight">Authorized Signature</p>
                                            </div>
                            ...
                            {/* Right Meta Pane */}
                            <div className="no-print p-5 flex-1 flex flex-col justify-between space-y-4">
                                <div className="space-y-3">
                                    <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 text-xs font-semibold">
                                        ...
                                        <span>Ready to Print / Download</span>
                                    </div>
                                    <h3 className="text-lg font-black text-slate-900">
                                        {fullName}
                                    </h3>
                                    <p className="text-xs text-slate-500 font-medium">
                                        Toggle sides, download badge image directly, or send to printer.
                                    </p>
                                ...
                                <div className="pt-3 border-t border-slate-100 flex flex-col gap-2">
                                    ...
                                    <button
                                        ...
                                        onClick={() => handleDownloadCardImage(!isFlipped ? "printable-card-front" : "printable-card-back", `${fullName.replace(/\s+/g, "_")}_ID_Card`)}
                                    ...
                                    >
                                        ...
                                        <span>Download ID Card Image</span>
                                    </button>
                                </div>
                            </div>
                        </div>
