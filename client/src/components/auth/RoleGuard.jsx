...
export default function RoleGuard({ children }) {
    ...
    useEffect(() => {
        ...
        setAllowed(canAccessPath(user.role, pathname));
        setChecked(true);
        ...
        return (
            <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-4">
                ...
                <p className="text-sm text-slate-500 max-w-sm">
                    Aapke role ({user?.role}) ko is page ko access karne ki permission nahi hai.
                </p>
                <button
                    onClick={() => router.push("/dashboard")}
                    className="mt-5 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl transition-colors"
                >
