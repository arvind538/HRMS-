...
export default function LoanAdvance() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="bg-white p-3.5 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-3 transition hover:shadow-md">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          {["ALL", "ACTIVE", "COMPLETED", "PENDING"].map((status) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold capitalize transition cursor-pointer ${statusFilter === status
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/75 border-b border-slate-200/80 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">
                    ...
                    <th className="py-3.5 px-6">Type & Reason</th>
                    <th className="py-3.5 px-6">Principal Amount</th>
                    <th className="py-3.5 px-6">Monthly EMI</th>
                  ...
                  {filteredLoans.map((item) => {
                    ...
                    return (
                      <tr key={id} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="py-4 px-6">
                          <span className="inline-block text-[11px] text-indigo-700 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-md font-semibold">
                  ...
                  <div key={id} className="p-4 space-y-3 bg-white hover:bg-slate-50/50 transition">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1.5">
                        <span
                          className={`text-xs font-semibold px-2.5 py-0.5 rounded-full border ${item.status === "ACTIVE"
                            ? "bg-emerald-50 text-emerald-700 border-emerald-200"
                            : item.status === "COMPLETED"
                              ? "bg-indigo-50 text-indigo-700 border-indigo-200"
                              : "bg-amber-50 text-amber-700 border-amber-200"
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-slate-100 animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5 text-xs">
              ...
              {/* Reason */}
              <div>
                <label className="font-semibold text-slate-700 block mb-1">Purpose / Reason</label>
                <textarea
                  rows={2}
                  required
