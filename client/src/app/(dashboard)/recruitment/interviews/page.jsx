...
export default function InterviewSchedulePage() {
  ...
  const fetchData = useCallback(async (isManual = false) => {
    ...
    try {
      ...
      const rawInt = Array.isArray(intRes?.data)
        ? intRes.data
        : Array.isArray(intRes?.data?.data)
          ? intRes.data.data
          : [];
      const rawCand = Array.isArray(candRes?.data)
        ...
        : Array.isArray(candRes?.data?.data)
          ? candRes.data.data
          : [];
      const rawEmp = Array.isArray(empRes?.data)
        ? empRes.data
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Interview schedule nahi hua.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const filteredInterviews = useMemo(() => {
    return interviews.filter((item) => {
      ...
      const intName = (item.interviewer?.name || "").toLowerCase();
      const round = (item.round || "").toLowerCase();
      const currentStatus = String(item.status || "scheduled").toLowerCase();
      const q = searchQuery.toLowerCase();

      const matchesSearch = candName.includes(q) || intName.includes(q) || round.includes(q);
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto w-full sm:w-auto">
          {[
            ...
            <button
              ...
              className={`px-3.5 py-1.5 rounded-2xl text-xs font-semibold whitespace-nowrap transition-all active:scale-95 ${statusFilter === tab.key
                    ...
                    >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <span className="text-[11px] text-slate-400">
                              {item.candidate?.jobPosition?.title || "Candidate"}
                            </span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-1.5 font-semibold text-slate-800">
                          ...
                          <span>{item.interviewer?.name || "—"}</span>
                        </div>
                      </td>

                      {/* Round */}
                      <td className="py-4 px-6">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold border capitalize ${roundBadge}`}>
                      ...
                      {/* Mode / Channel */}
                      <td className="py-4 px-6">
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-slate-50 border border-slate-200/80 text-xs font-semibold text-slate-700 capitalize">
                      ...
                      <td className="py-4 px-6" onClick={(e) => e.stopPropagation()}>
                        <select
                          ...
                          className={`pl-2.5 pr-6 py-1 border rounded-full text-[11px] font-bold capitalize appearance-none focus:outline-none cursor-pointer transition-all ${statusBadge.bg}`}
                        >
          ...
          <div className="bg-white w-full max-w-md h-full p-6 shadow-2xl overflow-y-auto flex flex-col justify-between animate-in slide-in-from-right duration-300 border-l border-slate-100">
            <div className="space-y-6">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <h2 className="text-xl font-bold text-slate-900 mt-0.5">
                    {selectedInterview.candidate?.name || "Session Details"}
                  </h2>
                </div>
                <button
              ...
              <div className="flex items-center gap-2">
                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border capitalize ${STATUS_BADGES[selectedInterview.status]?.bg || "bg-slate-100 text-slate-700"}`}>
                  {selectedInterview.status || "Scheduled"}
                </span>
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold border capitalize ${ROUND_BADGES[selectedInterview.round?.toLowerCase()] || ROUND_BADGES.screening}`}>
              ...
              <div className="bg-slate-50/80 rounded-3xl p-5 space-y-3.5 border border-slate-100 text-xs">
                ...
                <div className="flex justify-between py-1 border-b border-slate-200/60">
                  <span className="text-slate-400 font-medium">Panelist / Interviewer</span>
              ...
              {selectedInterview.meetingLink && (
                <div>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center justify-between p-3.5 bg-indigo-50/80 hover:bg-indigo-100 rounded-2xl border border-indigo-200/60 text-xs font-bold text-indigo-700 transition-colors"
                  >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Set panelist, assessment round, and meeting platform
                  </p>
                </div>
              </div>
              <button
