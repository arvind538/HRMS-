...
import { Clock, AlertTriangle, RefreshCw, Search, AlertCircle, Calendar } from 'lucide-react';
...
export default function LateComing() {
  ...
  const fetchLateRecords = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
  ...
  return (
    <div className="w-full space-y-6 font-sans pb-12 animate-in fade-in duration-300">
      {/* Top Header Card & Filters */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div className="bg-white p-5 rounded-3xl border border-slate-200/80 shadow-xs flex items-center gap-4 hover:border-amber-200 transition-all">
          <div className="p-3.5 bg-amber-50 text-amber-600 rounded-2xl"><Clock className="w-6 h-6" /></div>
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Late Count This Month</p>
            <h3 className="text-2xl font-bold text-slate-900 mt-0.5">{records.length}</h3>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-3xl border border-slate-200/80 shadow-xs flex items-center gap-3">
          <select
            ...
            className="flex-1 px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 cursor-pointer"
          >
          ...
          <input
            ...
            className="w-24 px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 font-mono"
          />
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col sm:flex-row gap-3 items-center justify-between">
        ...
        <button
          ...
          disabled={loading}
          className="p-2.5 bg-slate-50 border border-slate-200 hover:bg-slate-100 text-slate-600 rounded-xl transition shadow-2xs active:scale-95 cursor-pointer"
          title="Refresh Data"
        >
      ...
      <div className="bg-white rounded-3xl border border-slate-200/80 shadow-xs overflow-hidden">
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center text-slate-400">
            ...
            <p className="text-xs font-medium">Loading late arrival records...</p>
          </div>
        ) : error ? (
          ...
          <div className="py-20 text-center px-4">
            ...
            <h3 className="text-base font-semibold text-slate-800">Koi late-coming record nahi mila</h3>
            <p className="text-xs text-slate-400 mt-0.5">Selected month mein koi late arrival nahi hai.</p>
          </div>
        ) : (
                ...
                {filteredRecords.map((item) => {
                  ...
                  return (
                    <tr key={item._id} className="hover:bg-indigo-50/40 transition-all duration-150 group">
                      ...
                      <td className="py-4 px-6">
                        <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-amber-700 bg-amber-50 px-3 py-1 rounded-full border border-amber-200 shadow-2xs">
