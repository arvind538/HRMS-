...
import { Loader2, Save, ShieldCheck, Calendar, FileText } from "lucide-react";
...
export default function PFPage() {
  ...
  const fetchPFSettings = useCallback(async () => {
    setLoading(true);
    try {
      // Backend ke existing /settings endpoint se data fetch kar rahe hain
      const { data } = await api.get("/settings");
      setSettings(data);
      if (data.pfSettings) {
        setForm({
          pfNumber: data.pfSettings.pfNumber || "",
          monthlyDueDate: data.pfSettings.monthlyDueDate || "15th of every month",
          status: data.pfSettings.status || "active",
          remarks: data.pfSettings.remarks || ""
        });
      }
    } catch (err) {
      toast.error("Failed to load PF compliance settings.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSave = async (e) => {
    ...
    setSaving(true);
    try {
      // Existing /settings endpoint par pfSettings object merge karke bhej rahe hain
      await api.put("/settings", {
        ...settings,
        pfSettings: form
      });
      toast.success("PF compliance records saved successfully.");
      fetchPFSettings();
    } catch (err) {
      toast.error(err.response?.data?.message || "Failed to save PF records. Only administrators can edit this.");
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
              Manage monthly PF contribution filings, tracking records, and official due dates.
            </p>
          </div>
        </div>
      </div>
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-2 text-sm font-semibold text-slate-700">
              ...
              Establishment / PF Registration Number
            </label>
            <input
              type="text"
              value={form.pfNumber}
              ...
              placeholder="e.g. MH/BAN/1234567"
              required
            />
          </div>
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            placeholder="Add any specific notes regarding monthly PF challans or deposits..."
          />
        </div>
