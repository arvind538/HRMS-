...
// Roles allowed to view payroll analytics. Adjust if your role names differ.
...
export default function PayrollSummaryPage() {
    ...
    const fetchPayrollSummary = useCallback(async (isManual = false) => {
    ...
    const totalGross = useMemo(() => payrolls.reduce((acc, p) => acc + (p.grossSalary || 0), 0), [payrolls]);
    const totalDeductions = useMemo(() => payrolls.reduce((acc, p) => acc + (p.totalDeductions || 0), 0), [payrolls]);
    const totalNet = useMemo(() => payrolls.reduce((acc, p) => acc + (p.netSalary || 0), 0), [payrolls]);
    const paidCount = useMemo(() => payrolls.filter((p) => p.status === "paid").length, [payrolls]);

    const paidPercentage = useMemo(() => {
        return payrolls.length > 0 ? ((paidCount / payrolls.length) * 100).toFixed(1) : "0.0";
    ...
    const handleExportSnapshot = () => {
        ...
        link.href = url;
        link.download = `payroll-fiscal-summary-${year}-${new Date().toISOString().split("T")[0]}.json`;
        link.click();
    ...
    return (
        <div className="max-w-5xl mx-auto px-4 sm:px-2 lg:px-3 py-4 space-y-3 antialiased transition-all duration-300">
            {/* Control Header Card */}
                ...
                <div>
                    <div className="flex items-center gap-2.5">
                        ...
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-indigo-50 text-indigo-700 border border-indigo-200/70 shadow-xs">
                            <Sparkles size={13} className="text-indigo-600" /> FY {year}
                        </span>
                    </div>
                    <p className="text-xs sm:text-sm font-medium text-slate-500 mt-1">
                        Gross compensation liability, statutory deductions, and monthly salary disbursement cycles.
                    </p>
                </div>

                <div className="flex items-center gap-3 self-start sm:self-auto">
                    <div className="flex items-center gap-2 bg-slate-50 border border-slate-200/80 rounded-2xl px-3.5 py-2 shadow-xs">
            ...
            {/* Top 4 KPI Metrics (Clickable to redirect) */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                >
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="text-[11px] font-black text-slate-400 uppercase tracking-wider group-hover:text-indigo-600 transition-colors">Gross Total</span>
                    </div>
                    <h3 className="text-2xl sm:text-3xl font-black text-slate-900 font-mono truncate">
                        ₹{totalGross.toLocaleString("en-IN")}
                    </h3>
                    <p className="text-xs font-semibold text-slate-500 mt-1 flex items-center gap-1">
                        Pre-deduction allocation <ArrowUpRight size={14} className="opacity-0 group-hover:opacity-100 transition-opacity text-indigo-600" />
                    </p>
                </div>
                ...
                >
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="text-[11px] font-black text-rose-500 uppercase tracking-wider">Withholdings</span>
                    </div>
                    <h3 className="text-2xl sm:text-3xl font-black text-slate-900 font-mono truncate">
                        ₹{totalDeductions.toLocaleString("en-IN")}
                    </h3>
                    <p className="text-xs font-semibold text-rose-500 mt-1 flex items-center gap-1">
                        Taxes, PF & Insurance <ArrowUpRight size={14} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                    </p>
                </div>
                ...
                >
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="text-[11px] font-black text-emerald-600 uppercase tracking-wider">Net Outflow</span>
                    </div>
                    <h3 className="text-2xl sm:text-3xl font-black text-slate-900 font-mono truncate">
                        ₹{totalNet.toLocaleString("en-IN")}
                    </h3>
                    <p className="text-xs font-semibold text-emerald-600 mt-1 flex items-center gap-1">
                        <TrendingUp size={12} /> Direct bank credits <ArrowUpRight size={14} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                    </p>
                </div>

                <div
                    onClick={() => router.push(`/payroll?year=${year}&status=paid`)}
                    className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-sm hover:shadow-md hover:border-violet-300 transition-all duration-300 cursor-pointer group"
                >
                    <div className="flex items-center justify-between mb-4">
                        <div className="w-12 h-12 rounded-2xl bg-violet-50 text-violet-600 flex items-center justify-center border border-violet-100 group-hover:scale-110 transition-transform duration-300">
                            <CheckCircle2 size={22} />
                        </div>
                        <span className="text-[11px] font-black text-violet-600 uppercase tracking-wider">Settlement</span>
                    </div>
                    <h3 className="text-2xl sm:text-3xl font-black text-slate-900 font-mono">
                        {paidCount} <span className="text-xs font-normal text-slate-400">/ {payrolls.length}</span>
                    </h3>
                    <p className="text-xs font-semibold text-violet-600 mt-1 flex items-center gap-1">
                        {paidPercentage}% disbursement rate <ArrowUpRight size={14} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                    </p>
                </div>
            </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-5">
                    <div>
                        <h2 className="text-lg font-black text-slate-900 tracking-tight flex items-center gap-2">
                            ...
                            Monthly Net Salary Disbursed
                        </h2>
                        <p className="text-xs font-medium text-slate-500 mt-0.5">
                            Actual compensation volume released across calendar months of {year}
                        </p>
                    </div>
                    <div className="flex items-center gap-2 text-xs font-bold text-slate-600 bg-slate-50 px-3.5 py-1.5 rounded-xl border border-slate-200/60 self-start sm:self-auto font-mono">
                        ...
                        <span>Annual Target: ₹{totalNet.toLocaleString("en-IN")}</span>
                    </div>
                </div>

                {payrolls.length === 0 ? (
                    <div className="py-24 text-center space-y-1">
                        <p className="text-sm font-bold text-slate-700">No payroll statements logged</p>
                        <p className="text-xs font-medium text-slate-400">
                            No payroll distributions found for calendar year {year}.
                        </p>
                    </div>
                ) : (
