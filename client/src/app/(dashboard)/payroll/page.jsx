...
export default function PayrollPage() {
    ...
    const fetchPayrolls = async () => {
        ...
        try {
            ...
            console.error(err);
            toast.error("Payroll records load nahi ho paye.");
        } finally {
            setLoading(false);
        }
    };
    ...
    //  Robust Helpers to Extract Employee Details Safely
    const getEmpName = (row) => {
        if (row.employee && typeof row.employee === "object") {
            return row.employee.name || row.employee.fullName || row.employee.username || "Unknown Staff";
        }
        return row.employeeName || "Unknown Staff";
    };

    const getEmpId = (row) => {
        if (row.employee && typeof row.employee === "object") {
            return row.employee.employeeId || row.employee.code || row.employee._id?.toString().slice(-6) || "ID: —";
        }
        return row.employeeId || "ID: —";
    };

    const getEmpDept = (row) => {
        if (row.employee && typeof row.employee === "object") {
            return row.employee.department || row.employee.designation || row.employee.role || "Staff Member";
        }
        return row.department || "Staff Member";
    };
    ...
    const columns = [
        {
            ...
            label: "Employee",
            render: (row) => {
                const name = getEmpName(row);
                const empId = getEmpId(row);
                const dept = getEmpDept(row);

                return (
                    <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-indigo-600 to-violet-500 text-white flex items-center justify-center font-bold text-xs shadow-sm shadow-indigo-100 shrink-0">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                                <span className="text-[11px] font-mono font-bold text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded border border-indigo-100">
                                    {empId}
                                </span>
        ...
        },
        {
            ...
            render: (row) => {
                ...
                return (
                    <span
                        className={`inline-flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1 rounded-full border capitalize ${badgeStyles[st] || "bg-slate-50 text-slate-600 border-slate-200"
                            }`}
                    >
                        <span className={`w-1.5 h-1.5 rounded-full ${st === 'paid' ? 'bg-emerald-500' : st === 'processed' ? 'bg-blue-500' : 'bg-amber-500'
                            }`} />
                        {st}
                    </span>
                );
    ...
    return (
        <div className="max-w-[1400px] mx-auto p-4 sm:p-2 lg:p-2 space-y-3 animate-in fade-in duration-300 font-sans">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200/70 shadow-sm">
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className="p-2.5 border border-slate-200 rounded-xl text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-all duration-200 shadow-sm cursor-pointer"
                    ...
                    <Button
                        ...
                        loading={generating}
                        className="bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white rounded-xl px-4 py-2.5 font-medium shadow-md shadow-indigo-100 transition-all duration-200 flex items-center gap-2 cursor-pointer"
                    >
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-white p-5 rounded-2xl border border-slate-200/70 shadow-sm flex items-center justify-between hover:shadow-xl hover:border-indigo-200 transition-all duration-300 group">
                    <div>
                        <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider group-hover:text-indigo-600 transition-colors">Total Gross</p>
                ...
                <div className="bg-white p-5 rounded-2xl border border-slate-200/70 shadow-sm flex items-center justify-between hover:shadow-xl hover:border-rose-200 transition-all duration-300 group">
                    <div>
                ...
                <div className="bg-white p-5 rounded-2xl border border-slate-200/70 shadow-sm flex items-center justify-between hover:shadow-xl hover:border-emerald-200 transition-all duration-300 group">
                    <div>
                ...
                <div className="bg-white p-5 rounded-2xl border border-slate-200/70 shadow-sm flex items-center justify-between hover:shadow-xl hover:border-violet-200 transition-all duration-300 group">
                    <div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/70 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <input
                        ...
                        className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 w-24 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none transition-all font-mono"
                    />
                </div>
